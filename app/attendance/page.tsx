...
export default function AttendancePayroll() {
  ...
  const handleMarkAllPresent = async () => {
    ...
    if (!filters.project || filters.project === "all" || filters.project === "") {
      toast({
        ...
        description: "Please select a specific project before marking bulk attendance. This ensures attendance is recorded for the correct project.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleBulkAttendanceMarking = async (status?: string) => {
    ...
    if (!filteredEmployees.length) {
      toast({
        title: "No Employees",
        description: "No employees available to mark attendance for the selected project.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Show results
      let message = `Successfully marked ${successCount} employees as ${attendanceStatus.toLowerCase()} for project "${filters.project}".`;
      if (skippedCount > 0) {
