              ...
              <tbody>
                ...
                <tr>
                  ...
                  {Array.from({ length: numberOfPeriods }, (_, index) => {
                    ...
                    return (
                      <td key={`revenue-${index}`} className="border border-gray-300 px-4 py-2 text-right">
                        {formatCurrency(calculatePeriodRevenue(periodKey))}
                      </td>
                    );
                  })}
                ...
                <tr className="bg-gray-100 font-bold">
                  ...
                  {Array.from({ length: numberOfPeriods }, (_, index) => {
                    const periodKey = `period${index + 1}`;
                    return (
                      <td key={`total-revenue-${index}`} className="border border-gray-300 px-4 py-2 text-right">
                        {formatCurrency(calculatePeriodRevenue(periodKey))}
                      </td>
                    );
                  })}
                ...
                {projectTrades.map((trade: any) => {
                  ...
                  return (
                    <tr key={trade.id}>
                      ...
                      {periodPayrolls.map((payroll, index) => (
                        <td key={`trade-${trade.id}-period-${index}`} className="border border-gray-300 px-4 py-2 text-right">
