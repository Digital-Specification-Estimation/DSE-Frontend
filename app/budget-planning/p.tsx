...
export default function BudgetPlanning() {
  ...
  const exportProjectReport = async (project: any) => {
    try {
      ...
      doc.text(`Budget: $${project.budget?.toLocaleString() || "0"}`, 14, 30);
      ...
      if (project.trade_positions?.length > 0) {
        const tableData = project.trade_positions.map(
          (trade: any, index: number) => [
            ...
            trade.work_days || "N/A",
            `$${(trade.daily_planned_cost || 0).toLocaleString()}`,
            `$${(
        ...
        doc.text(
          `Total Planned Cost: $${totalPlanned.toLocaleString()}`,
          14,
  ...
  const exportAllProjectsReport = async () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text(
        `Total Budget Across Projects: $${totalBudget.toLocaleString()}`,
        14,
        yPosition
      );
      yPosition += 10;
      doc.text(
        `Total Planned Costs: $${totalPlannedCost.toLocaleString()}`,
        14,
        yPosition
      );
      yPosition += 10;
