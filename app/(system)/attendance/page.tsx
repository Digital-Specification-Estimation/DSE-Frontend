...
export default function AttendancePayroll() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const leaveCount = employees.filter(
        (e) => e.attendance_status === "on_leave"
      ).length;
      ...
      yPosition = addText(`Absent: ${absentCount}`, margin, yPosition);
      yPosition = addText(`On Leave: ${leaveCount}`, margin, yPosition);
      ...
      employees.forEach((employee) => {
        ...
        const rowData = [
          ...
          employee.attendance_status?.charAt(0).toUpperCase() +
            employee.attendance_status?.slice(1) || "N/A",
      ...
      toast({
        title: "Error",
        description:
          "Failed to generate the attendance report. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const leaveEndIndex = Math.min(
    leaveStartIndex + leaveItemsPerPage,
    totalLeaveItems
  );
  const currentLeaveItems = filteredEmployees.slice(
    leaveStartIndex,
    leaveEndIndex
  );
              ...
              {totalLeavePages > 0 && (
                <div className="flex items-center justify-between mt-4 px-4 py-3 bg-white border rounded-b-md">
                  <div className="flex items-center space-x-2">
                    <p className="text-sm text-gray-700">
                      Showing{" "}
                      <span className="font-medium">{leaveStartIndex + 1}</span>{" "}
                      to <span className="font-medium">{leaveEndIndex}</span> of{" "}
                      <span className="font-medium">{totalLeaveItems}</span>{" "}
                      employees
                    </p>
                    <Select
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    {/* Page Numbers */}
                    {Array.from(
                      { length: Math.min(5, totalLeavePages) },
                      (_, i) => {
                        let pageNum;
                    ...
                    {totalLeavePages > 5 &&
                      leaveCurrentPage < totalLeavePages - 2 && (
                        <span className="px-2 text-gray-500">...</span>
                      )}
                    ...
                    <Button
                      ...
                      onClick={() => goToLeavePage(leaveCurrentPage + 1)}
                      disabled={
                        leaveCurrentPage === totalLeavePages ||
                        totalLeavePages === 0
                      }
                    >
                      <span className="sr-only">Go to next page</span>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2 mb-4">
                ...
                >
                  {isMarkingAllPresent && bulkAttendanceStatus === "Present" ? (
                    <>
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                      Marking Present...
                    </>
                ...
                >
                  {isMarkingAllPresent && bulkAttendanceStatus === "Absent" ? (
                    <>
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                      Marking Absent...
                    </>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <CalendarComponent
                    ...
                    disabled={(date) => {
                      ...
                      const validEmployeesForDate = filteredEmployees.filter(
                        (emp) => isDateValid(date, emp)
                      );
            ...
            <div
              className={`p-3 rounded-lg ${
                bulkAttendanceStatus === "Present" ? "bg-green-50" : "bg-red-50"
              }`}
            >
              <div
                className={`text-sm ${
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <CalendarComponent
                    ...
                    onSelect={(date) => date && setSelectedIndividualDate(date)}
                    disabled={(date) => {
            ...
            {selectedEmployee && (
              <div className="bg-green-50 p-3 rounded-lg">
                <div className="text-sm text-green-800">
                  <strong>Action:</strong> Mark {selectedEmployee.username} as{" "}
                  {pendingAttendanceAction?.status} for{" "}
                  {selectedIndividualDate.toLocaleDateString()}
                </div>
